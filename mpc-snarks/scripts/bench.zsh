...
case $infra in
    hbc|spdz|gsz)
        ...
        do
          ...
          if [ $i -eq 0 ]
          then
            $BIN -p $proof -c squaring --computation-size $size mpc --hosts $NETWORK_CONFIG --party $i --alg $infra > logs0.txt & #| rg "End: *$LABEL" | rg -o '[0-9][0-9.]*.s' &
            pid=$!
          else
            $BIN -p $proof -c squaring --computation-size $size mpc --hosts $NETWORK_CONFIG --party $i --alg $infra > logs1.txt &
            pid=$!
          fi
          PROCS+=($pid)
        done
