...
pub trait MpcFpParameters<F: PrimeField<BigInteger = T>, S: FieldShare<F>, T: _BigInteger>: FieldParameters<BigInteger = &'static MpcBigInteger<F, S, T>> {}
...
impl<F: PrimeField<BigInteger = T>, S: FieldShare<F>, T: _BigInteger + 'static> FftParameters for MpcFrParameters<F, S, T> {
    ...
    const POWERS_OF_ROOTS_OF_UNITY: &'static [&'static Self::BigInteger] = &[ //];
        &'static MpcBigInteger::<F, S, T>{val: <<F as PrimeField>::Parameters as FftParameters>::POWERS_OF_ROOTS_OF_UNITY[0], _marker: PhantomData},
    ...
    #[rustfmt::skip]
    const TWO_ADIC_ROOT_OF_UNITY: &'static MpcBigInteger<F, S, T> = &'static MpcBigInteger::<F, S, T>{
...
impl<F: PrimeField<BigInteger = T>, S: FieldShare<F>, T: _BigInteger> FieldParameters for MpcFrParameters<F, S, T> {
    ...
    #[rustfmt::skip]
    const GENERATOR: &'static MpcBigInteger<F, S, T> = &'static MpcBigInteger::<F, S, T>{
    ...
    #[rustfmt::skip]
    const MODULUS: &'static MpcBigInteger<F, S, T> = &'static MpcBigInteger::<F, S, T>{
    ...
    #[rustfmt::skip]
    const MODULUS_MINUS_ONE_DIV_TWO: &'static MpcBigInteger<F, S, T> = &'static MpcBigInteger::<F, S, T>{
        val: <<F as PrimeField>::Parameters as FieldParameters>::MODULUS_MINUS_ONE_DIV_TWO,
        _marker: PhantomData, 
    };
    #[rustfmt::skip]
    const R: &'static MpcBigInteger<F, S, T> = &'static MpcBigInteger::<F, S, T>{
        val: <<F as PrimeField>::Parameters as FieldParameters>::R,
        _marker: PhantomData, 
    };
    #[rustfmt::skip]
    const R2: &'static MpcBigInteger<F, S, T> = &'static MpcBigInteger::<F, S, T>{
    ...
    #[rustfmt::skip]
    const T: &'static MpcBigInteger<F, S, T> = &'static MpcBigInteger::<F, S, T>{
    ...
    #[rustfmt::skip]
    const T_MINUS_ONE_DIV_TWO: &'static MpcBigInteger<F, S, T> = &'static MpcBigInteger::<F, S, T>{
...
mod tests {
    use super::*;
    use snarkvm_curves::bls12_377::Fq; // TODO: try all fields
