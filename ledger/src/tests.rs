...
finalize execute:
    ...
    function set_operator:
        ...
        hash.bhp256 r5 into r6 as field;
        cast staker_v{i}_multisig_b.aleo 3u8 r6 r2 0u32 into r7 as Operation;
        ...
        output r10 as staker_v{i}_multisig_b.aleo/set_operator.future;
    ...
    function add_admin:
        ...
        cast staker_v{i}_multisig_b.aleo 1u8 r2 r1 1200u32 into r3 as Operation;
        call multisig_v1.aleo/execute r3 into r4;
        call staker_v1_b.aleo/set_admin r0 true into r5;
        async add_admin r4 r5 into r6;
        output r6 as staker_v{i}_multisig_b.aleo/add_admin.future;
    ...
    function remove_admin:
        ...
        cast staker_v{i}_multisig_b.aleo 2u8 r4 r1 0u32 into r5 as Operation;
        ...
        output r8 as staker_v{i}_multisig_b.aleo/remove_admin.future;
    ...
    // Deploy base programs.
    println!("Deploying program: {}", prelim_program_1.id());
    let deployment = ledger.vm.deploy(&private_key, &prelim_program_1, None, 0, None, rng).unwrap();
    let block =
    ...
    ledger.advance_to_next_block(&block).unwrap();
    println!("Deploying program: {}", prelim_program_2.id());
    let deployment = ledger.vm.deploy(&private_key, &prelim_program_2, None, 0, None, rng).unwrap();
    let block =
        ledger.prepare_advance_to_next_beacon_block(&private_key, vec![], vec![], vec![deployment], rng).unwrap();
    assert!(block.aborted_transaction_ids().is_empty());
    ledger.advance_to_next_block(&block).unwrap();
    println!("Deploying program: {}", prelim_program_3.id());
    let deployment = ledger.vm.deploy(&private_key, &prelim_program_3, None, 0, None, rng).unwrap();
    let block =
        ledger.prepare_advance_to_next_beacon_block(&private_key, vec![], vec![], vec![deployment], rng).unwrap();
    assert!(block.aborted_transaction_ids().is_empty());
    ledger.advance_to_next_block(&block).unwrap();

    // Add lots of programs to the cache to test eviction.
    {
        // Acquire process ref.
        let process_ref = ledger.vm().process();
        // Acquire write lock.
        let mut process = process_ref.write();
        ...
        // Add MAX_STACKS dummy deployments to test cache eviction.
    ...
    println!("Deploying program: {}", root_1.id());
    let deployment = ledger.vm.deploy(&private_key, &root_1, None, 0, None, rng).unwrap();
    let block =
        ledger.prepare_advance_to_next_beacon_block(&private_key, vec![], vec![], vec![deployment], rng).unwrap();
    assert!(block.aborted_transaction_ids().is_empty());
    ledger.advance_to_next_block(&block).unwrap();

    // Add 10 more dummy programs to the cache to test eviction.
    for i in 0..10 {
        let program = Program::<CurrentNetwork>::from_str(&format!(
            r"
...
function compute:"
    ...
    println!("Deploying program: {}", root_2.id());
    let deployment = ledger.vm.deploy(&private_key, &root_2, None, 0, None, rng).unwrap();
    let block =
        ledger.prepare_advance_to_next_beacon_block(&private_key, vec![], vec![], vec![deployment], rng).unwrap();
    assert!(block.aborted_transaction_ids().is_empty());
    ledger.advance_to_next_block(&block).unwrap();
    assert_eq!(ledger.vm().process().read().num_stacks_in_memory(), CurrentNetwork::MAX_STACKS);
    assert!(ledger.vm().process().read().contains_program_in_cache(root_2.id()));
}
